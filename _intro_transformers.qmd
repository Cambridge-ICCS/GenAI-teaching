...
:::
:::{style="font-size: 0.65em; color: #888; margin-top: 0.5em;"}
3 tokens
:::
:::
::::
...
<!-- ::: {style="text-align: center; font-size: 0.6em; color: #888; margin-top: 0.4em;"}
::: -->
:::{.notes}
...
::: {.notes}
The transformer block is where the magic happens. The key innovation is self-attention: rather than processing tokens in sequence (like an RNN), every token attends to every other token in parallel. This lets the model capture long-range dependencies and scales well on modern hardware.

The MLP layers capture facts and rules of language. 
:::
...
::: {.notes}
The unembedding matrix is applied at every position, but only the last token's logits are sampled — it's the only one that has seen the full context of the sequence.
:::
...
- Each forward pass starts from scratch
:::

::: {.notes}
This has profound implications for how you use LLMs. Long conversations get expensive because the full history is re-processed every time. Context length limits are a hard constraint. Techniques like RAG, summarisation, and memory modules are all workarounds for this fundamental limitation.
:::
