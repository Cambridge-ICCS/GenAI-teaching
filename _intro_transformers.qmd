...
::: {style="font-size: 0.75em; color: #555; margin-top: 1em; line-height: 1.8;"}
- **Q (Query)**: what this token is looking for
- **K (Key)**: what each token has to offer
- **V (Value)**: what gets passed forward if attended to
- $\sqrt{d_k}$: scaling factor to stabilise gradients
:::
:::
::: {.column width="50%"}
::: {style="font-size: 0.75em; color: #555; margin-top: 2em; line-height: 1.8;"}
1. Dot product Q·Kᵀ gives a score for every token pair
2. Softmax converts scores to weights (sum to 1)
3. Weighted sum of V vectors gives the new representation
:::
:::
::::

::: {.notes}
Q, K, V are learned linear projections of the input embeddings. The dot product QKᵀ measures compatibility between every pair of tokens — how much should token i attend to token j? Dividing by √d_k prevents the dot products from growing large in high dimensions, which would push softmax into regions with near-zero gradients. The output is a weighted sum of value vectors, where the weights are the attention scores.
:::
